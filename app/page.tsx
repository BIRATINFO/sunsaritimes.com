...
export default async function Page() {
  ...
  const roadblockRes = await fetch(`${backend_uri}/api/roadblocks/network/sunsaritimes.com`, options)
  ...
  const [heroData, categoryBlocks] = await Promise.all([
    fetch(`${backend_uri}/api/posts/featured-news/sunsaritimes.com`, options)
      .then(res => res.ok ? res.json() : { post: [] })
      .then(data => data?.post || [])
      .catch(() => []),

    fetch(`${backend_uri}/api/posts/homepage/sunsaritimes.com`, options)
      .then(res => res.ok ? res.json() : { post: [] })
      .then(data => data?.post || [])
      .catch(() => [])
  ])
